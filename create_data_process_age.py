...
for dir_name in os.listdir(data_path):
    ...
    result_path_train = "data_process_age/train/{}".format(dir_name)
    if not os.path.exists(result_path_train):
        os.makedirs(result_path_train)

    result_path_validation = "data_process_age/validation/{}".format(dir_name)
    if not os.path.exists(result_path_validation):
        os.makedirs(result_path_validation)

    result_path_test = "data_process_age/test/{}".format(dir_name)
    ...
    for file_name in os.listdir(tmp_path):
        ...
        if TEMP1 < 200 and x <= 20:
            shutil.copy(file_path, result_path_validation)
            TEMP1 += 1
        elif TEMP2 < 100 and (x > 20 and x <= 30) :
